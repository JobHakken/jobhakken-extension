...
| **Oracle Recruiting Cloud** (Hologic, JPMC) | ✅ live                  | email + terms (Knockout SPA, no CAPTCHA) | Gate mapped (`#primary-email-0`, `#legal-disclaimer-checkbox`, Next); gate-advance still WIP (Knockout re-render races). Form not yet reached. |
...
| **Dayforce** (York Space)                   | ✅ live (ASRC link dead) | account/sign-in (no CAPTCHA)             | Job loads; apply needs sign-in. Automatable with account creation (next).                                                                      |
| **BrassRing/Kenexa** (General Atomics)      | ✅ live                  | cookie + account (no CAPTCHA)            | Loads behind cookie consent; apply needs account. Automatable (next).                                                                          |
