...
const loc = await p.evaluate(() => {
  const lab = [...document.querySelectorAll('label,.select__label,[class*=label]')].find((l) =>
    /^location/i.test((l.textContent || '').trim()),
  );
  const cont = lab?.closest('div,fieldset')?.parentElement || lab?.closest('div');
  const sv = cont?.querySelector('[class*=singleValue],[class*=single-value],[class*=multiValue]');
  return {
    display: (sv?.textContent || '').trim(),
    containerText: (cont?.innerText || '').replace(/\s+/g, ' ').slice(0, 80),
  };
});
console.log('autofill:', r?.filled, '/', tgt.total);
console.log('Location field →', JSON.stringify(loc));
console.log(
  loc.display && /austin|texas|tx/i.test(loc.display)
    ? 'LOCATION FILLED ✓'
    : loc.display
      ? 'picked: ' + loc.display
      : 'Location still empty',
);
await p.screenshot({ path: '/tmp/shots/location.png', fullPage: false }).catch(() => {});
await ctx.close();
